...
- Delivered so far:
  ...
  - **Step 3a (light restyle + archive-first header):** replaced the dark-green hero with a **light identity header** in the locked system (avatar, name, status, company/title, source badge, last-edited, added/updated metadata) and an **archive-first action row** — Share · Archive/Restore · Favorite · ⋯ menu with **Delete permanently** (native `<details>`, no JS). Dropped the off-design marketing copy / detail-stat cards / record-health panel.
- Remaining (step 3b — the heavier pieces, own follow-up):
  - True **320px sticky left-rail two-pane** (identity/badges/metadata as a side rail beside the tabbed right pane) and **inline-edit** (auto-save on blur) replacing the single big `updateContact` form. Inline-edit is the largest interaction rewrite; the current Save-button edit form remains in the Details tab until then.
- Implementation Notes:
