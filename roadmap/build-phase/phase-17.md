...
- Deviations (follow-ups, noted): native `<select>` label pills instead of custom dropdown menus; no phone country-code selector; single structured address (not multi-address yet); no unsaved-changes modal / save-error toast (Save-disabled-until-valid is the gate); not yet wrapped in the persistent sidebar `AppShell` — that shell is extracted in **P17-02** and this form should adopt it then.
