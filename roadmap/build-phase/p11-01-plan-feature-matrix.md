...
> **Status: FROZEN (2026-06-10).** For complete lifecycle policies (group dissolution procedures, payment failure cascade, member join/leave rules, downgrade data fate, and notification policies) see `lifecycle-policies.md`, which extends and fills the gaps in this document. Tiers, entitlement values, cross-tier sharing rules, and downgrade paths are agreed. Product decisions resolved: member counts (Family 6 / Teams 25), PRO app-password limit (5), PLUS removal (no subscribers), merge-undo mechanism (existing `MergeDecision`, P10-05). Three non-blocking items remain flagged in the "Still open" subsection (vCard bulk import, Teams expansion, Teams shared-book CardDAV) — none gate P11-02/P11-03. P11-02 schema work may proceed.
