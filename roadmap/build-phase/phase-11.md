...
- Delivered:
  ...
  - **Per-contact history display-capped for Free** in `/api/contacts/[id]/history` via `historyDisplayCap` (Free shows the last 3; keeps 10 — see P11-05); paid tiers (null cap) stay uncapped with cursor pagination.
...
- Delivered:
  - `scripts/prune-activity-retention.mjs` + `npm run prune:activity` (and `--dry-run`). Per-user (not bulk), idempotent, safe to re-run. Deletes `ActivityEvent` rows older than the user's plan window **except the last 20 events per contact** (the floor — recent history is never lost). Windows: **Pro 365d, Family 90d**; **Free (0) and Teams (null) skipped**. Implemented with a `ROW_NUMBER() OVER (PARTITION BY contactId …)` window query; contact-less events get the window with no floor.
  ...
  - **Retention spec (decided 2026-06-10):** **Pro 365 / Family 90** (Family < Pro by per-seat economics — the one exception to "everything in Pro"), Teams unlimited. **Free: no time window — count-capped to the last 10 per contact (kept), History tab shows the last 3 (teaser).** Floor (kept) = last **20 per contact** for paid, **10** for Free; display cap = **3** for Free, all for paid. Retention physically deletes events and bounds both the global feed and per-contact history; the matrix's "Pro unlimited per-contact history" is superseded. Entitlements add `historyFloorPerContact` (kept) + `historyDisplayCap` (shown). All documented in `p11-01-plan-feature-matrix.md`.
  - **Downgrade handling:** the window/floor derive from the user's *current* active plan at run time, so a downgraded user is pruned to the new window on the next nightly run.
  - Verified: tsc + lint + build green; `--dry-run` against the DB runs the floor-aware window query cleanly.
