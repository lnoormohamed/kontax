...
- Delivered:
  ...
  - **Retention spec (decided 2026-06-10):** **Pro 365 / Family 90** (Family < Pro by per-seat economics — the one exception to "everything in Pro"), Teams unlimited, Free no-window. Floor = last **20 per contact** (Free last **10**). Retention physically deletes events and bounds **both** the global feed and per-contact history; the matrix's "Pro unlimited per-contact history" is superseded. Entitlements add `historyFloorPerContact`; the per-contact history route caps Free to its floor and paid tiers show all retained. All documented in `p11-01-plan-feature-matrix.md`.
