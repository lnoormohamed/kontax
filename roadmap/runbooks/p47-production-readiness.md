...
| 16 | **Uptime monitoring** | ◑ **2026-07-07**: Uptime Kuma (LXC 131, systemd `uptime-kuma.service`, SQLite `/opt/uptime-kuma/data/kuma.db`). App already monitored (id 14 `getkontax.com/api/health`, 300s/3-retry). **Added id 17 `Kontax Media (MinIO)` → `media.getkontax.com/minio/health/live`** (cloned id 14's config; verified UP 200/279ms; DB backup `kuma.db.bak-p47-20260707`). **Cron monitor DONE 2026-07-07**: created Kuma **Push** monitor id 18 `Kontax Cron (LXC 152)` (interval 1800s, 1 retry @900s → down after ~45 min of silence; token in the ops vault / kuma.db; DB backup `kuma.db.bak-cronmon-20260707`). LXC 152 crontab (`/etc/cron.d/kontax`, backup `/root/kontax-crontab.bak-20260707`) now pings `http://192.168.1.58:3001/api/push/<token>` every 15 min — verified UP. **Alerting still DEFERRED (user): Telegram** channel to a later plan; note there are **zero notification providers in Kuma for ANY project** — so all monitors (incl. this one) are dashboard-only until Telegram is wired. App + media + cron all monitored now | Kuma SQLite + push monitor + crontab heartbeat | P47-12 |
...
6. **Uptime Kuma** (P47-12 residual) — app + media + **cron heartbeat (id 18)
   now monitored** (2026-07-07). Only **Telegram alerting DEFERRED to a future
   plan** (user); until then all monitors are dashboard-only.
