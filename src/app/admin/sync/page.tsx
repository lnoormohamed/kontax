...
export default async function AdminSyncPage({
  ...
  const data = admin.capabilities["sync.view"]
    ? await loadAdminSyncOverview({
  ...
  const bannerTone = data
    ? adminAttentionMeta(worstTone(data.summary.actionRequired, data.summary.warningConnections))
    : null;
    ...
    <>
      ...
      <div className="adm-content">
        <div className="ad-page">
          {!admin.capabilities["sync.view"] || !data ? (
            <AdminAccessPanel
              title="Sync ops is unavailable"
              ...
              <div className="ad-sync-provider-grid">
                {data.providerCards.map((card) => {
                  const tone = adminAttentionMeta(
                    card.tone === "action" ? "action" : card.tone === "warning" ? "warning" : card.tone === "watch" ? "watch" : "healthy",
                  );
                  return (
