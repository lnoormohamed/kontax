...
function ExportInfoCard({ contactsCount }: { contactsCount: number }) {
  return (
    <section className="rounded-2xl border border-[#d8ddd6] bg-[#fbfcf9] px-5 py-[18px]">
      <div className="flex items-center gap-2.5">
        <WorkspaceIcon name="download" size={16} className="text-[#17352e]" />
        <span className="text-[11px] font-bold uppercase tracking-[0.13em] text-[#8b938c]">Export ready</span>
      </div>
      <div className="mt-3 text-[24px] font-bold tracking-[-0.01em] text-[#1d2823] tabular-nums">
        {contactsCount}
      </div>
      <div className="mt-1 text-[13px] text-[#5c655e]">active contacts available to export today.</div>
      <div className="mt-3 rounded-[14px] bg-white px-4 py-3 text-[13px] leading-[1.55] text-[#5c655e]">
        Export mode stays focused on download tasks: CSV for spreadsheets and vCard for address books.
      </div>
    </section>
  );
}
    ...
    <AppShell account={account} mobileTitle="Import & Export" mobileBackHref="/contacts" mobileBackLabel="Contacts">
      <div className="mx-auto grid w-full max-w-[1060px] content-start gap-4 px-4 pb-24 pt-6 md:px-9 md:pt-8">
        <div>
          ...
          <p className="mt-2 text-[14.5px] leading-[1.5] text-[#5c655e]">{introCopy}</p>
        </div>
          ...
          {/* Import / Export segmented tab stays visible on every breakpoint so the page mode is explicit. */}
          <div className="flex overflow-hidden rounded-[11px] border border-[#d8ddd6] bg-white">
            <a
          ...
          <div className={`${presetsMode ? "hidden" : "grid"} items-start gap-4 ${exportMode ? "" : "xl:grid-cols-[minmax(0,1.85fr)_minmax(0,1fr)]"}`}>
            ...
            <div className={`grid content-start gap-4 ${!exportMode && activeTab === "import" ? "hidden xl:grid" : ""}`}>
              <ExportCard hasContacts={contactsCount > 0} premiumExport={planSummary.entitlements.premiumExportEnabled} />
              {exportMode ? (
                <ExportInfoCard contactsCount={contactsCount} />
              ) : (
          ...
          {/* Import history stays scoped to import mode so export remains task-focused. */}
          {!exportMode ? (
